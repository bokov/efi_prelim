...
.outcomenames <- v(c_mainresponse);
.currentscript <- current_scriptname('clinstar_prelim.Rmd');
# limit the data to items of interest
tb2 <- subset(tb2,predictor == 'Frailty, age:65+');
tb3 <- subset(tb3,Predictor == 'Frailty, age:65+');
...
```{r survcurves,message=FALSE,results='asis'}
panderOptions('knitr.auto.asis', FALSE);
for(jj in fits[.outcomenames]) {
  with(jj,{
    message(dispname);
    cat('\n###',dispname,'\n\n');
    print(multiplot$`65+` + ggtitle(''));
    cat('\n\n\n');
    modelsummary[c('Frailty, age:65+','Patient Age, 65+'),] %>% rbind %>% pander;
    cat('\n******\n');
  })};
```
...
```{r tb2}
panderOptions('knitr.auto.asis', TRUE);
tb2[,c('Outcome','β^ (95% CI)','fold-change (95% CI)','SE','Z','P, adjusted')] %>%
