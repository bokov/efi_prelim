---
title: "Development and validation of an EHR-based frailty index for predicting infections, non-operative trauma, cardiac complications, and patient safety indicators"
...
abstract: | 
  ...
  predict patient safety indicators (PSIs) using only codes and numeric values 
  ...
  complications, and to a lesser extent the ocurrence of any PSI.
...
Improving patient safety is a key goal for every health system. A simple and 
rapid frailty assessment would help clinicians can tailor interventions to avoid 
poor patient outcomes. Our objective was to evaluate a deficit-accumulation
electronic frailty index (eFI) as a predictor of preventable infections, 
non-operative trauma, cardiac complications, and general patient safety 
indicators (PSIs) and then validate it against hold-out data.
...
```{r consort,fig.cap=NULL}
#grViz(consortgv);
grViz(consortgv) %>% DiagrammeRsvg::export_svg() %>% charToRaw %>% rsvg::rsvg_png(file='foo.png');
...
The four outcomes we predicted with eFI were hospital-acquired infections,
non-operative hospital-acquired trauma, cardiac complications, and the 
occurrence of one or more of these or one or more PSIs in 
general as defined by @southern17. Following principles of implementation 
science [@meissner20a], we drew on the expertise of the clinical stakeholders on 
our team (CT and SE) to select outcomes that are meaningful, relevant, and 
useful in guiding clinical care planning. To detect these outcomes in EHR data we 
used ICD-10 code mappings developed by @southern17. The advantage of these PSI 
definitions is that they leverage the specificity of ICD10 codes and the 
uniform denominator greatly simplifies calculation. 
...
curves in Figures \@ref(fig:kmpsicap)-\@ref(fig:kmcardcap).
...
Figures \@ref(fig:kmpsicap)-\@ref(fig:kmcardcap) , split on eFI 
(Frail if above 0.19, non-Frail otherwise). Each plot is followed by
...
`r scales::label_percent(accuracy=0.1)(predict(fitsval$vi_c_psi$models$Frailty,newdata=list(a_t0=0,a_t1=30,xx=NA,a_efi=0),type='expected'))` 
baseline chance of experiencing a PSI during a 30-day period, 
then if their eFI were 0.1 their chance would rise to
`r scales::label_percent(accuracy=0.1)(predict(fitsval$vi_c_psi$models$Frailty,newdata=list(a_t0=0,a_t1=30,xx=NA,a_efi=0.1),type='expected'))` 
and if their eFI were 0.2, just above our operational definition of frailty,
it would be 
`r scales::label_percent(accuracy=0.1)(predict(fitsval$vi_c_psi$models$Frailty,newdata=list(a_t0=0,a_t1=30,xx=NA,a_efi=0.2),type='expected'))` 


```{r kmpsi, fig.cap=NULL, message=FALSE,collapse='true',results='asis'}
panderOptions('knitr.auto.asis', FALSE);
with(fitsval$vi_c_psi,{
  message(dispname);
  plotsurv01(survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num,data=data)
             ,survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num
                      ,data=fits$vi_c_psi$data)
...
```
```{r kmpsicap, fig.cap='Any PSI',fig.height=0.1}
.oldmar <- par('mar'); par(mar=c(0,0,0,0));
plot(0,type='n',axes=F,xlab=NA,ylab=NA,cex=0.01);
par(mar=.oldmar);
```
...
panderOptions('knitr.auto.asis', FALSE);
.subfig <- 0;
with(fitsval$vi_c_hospinfect,{
  message(dispname);
  plotsurv01(survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num,data=data)
             ,survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num
                      ,data=fits$vi_c_hospinfect$data)
             ,labs=c('Non-Frail','Frail',
                     '\nNon-Frail\n(development)','\nFrail\n(development)')
             ,title=dispname) %>% print; 
  pander(cphunivar(models$Frailty,3,3),row.names=F);
...
```
```{r kminfcap, fig.cap='Hospital acquired infections.',fig.height=0.1}
.oldmar <- par('mar'); par(mar=c(0,0,0,0));
plot(0,type='n',axes=F,xlab=NA,ylab=NA,cex=0.01);
par(mar=.oldmar);
```
...
panderOptions('knitr.auto.asis', FALSE);
with(fitsval$vi_c_hosptrauma,{
  message(dispname);
  plotsurv01(survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num,data=data)
             ,survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num
                      ,data=fits$vi_c_hosptrauma$data)
             ,labs=c('Non-Frail','Frail',
                     '\nNon-Frail\n(development)','\nFrail\n(development)')
             ,title=dispname) %>% print; 
  pander(cphunivar(models$Frailty,3,3),row.names=F);
...
```
```{r kmtracap, fig.cap='Non-operative trauma.',fig.height=0.1}
.oldmar <- par('mar'); par(mar=c(0,0,0,0));
plot(0,type='n',axes=F,xlab=NA,ylab=NA,cex=0.01);
par(mar=.oldmar);
```
...
```{r kmcard, fig.cap=NULL, message=FALSE,collapse='true',results='asis'}
panderOptions('knitr.auto.asis', FALSE);
.subfig <- 0;
with(fitsval$vi_c_cardiac,{
  message(dispname);
  plotsurv01(survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num,data=data)
             ,survfit(Surv(a_t0,a_t1,xx)~Frail,id=patient_num
                      ,data=fits$vi_c_cardiac$data)
             ,labs=c('Non-Frail','Frail',
                     '\nNon-Frail\n(development)','\nFrail\n(development)')
             ,title=dispname) %>% print; 
  pander(cphunivar(models$Frailty,3,3),row.names=F);
...
```
```{r kmcardcap, fig.cap='Cardiac complications.',fig.height=0.1}
.oldmar <- par('mar'); par(mar=c(0,0,0,0));
plot(0,type='n',axes=F,xlab=NA,ylab=NA,cex=0.01);
par(mar=.oldmar);
```
...
`r valnote("All models had a C-statistic above 0.7, with cardiac complications 
exceeding 0.8. The proportional hazard assumption was satisfied for all outcomes 
except for all patient safety indicators combined (Table \\@ref(tab:tb3)). 
Though statistically significant, this time-dependent deviation from 
proportionality is modest and is in the direction of increasing divergence with 
time (Figure S1).")`
...
all the outcomes passed this test with the exception of the all-PSI model.')`
...
development results, a slope less than indicates lowered sensitivity and a slope
of zero would imply no correlation between the development and validation 
...
over-esimated the sensitivity of eFI as a predictor. However, except for
all-PSI, the slopes were above 0.9.')`
...
lapply(valresults[.outcomenames]
  ...
  pander(row.names=F,caption='(\\#tab:lmdiscrim) Validation against out-of sample predictions from the development model.',keep.line.breaks=T)
```
...
deficit-accumulation framework and used it to predict PSIs in
...
In other words, the design of this study mimics a scenario where an existing 
patient is seen for the first time after clinical deployment of eFI and the goal
of the validation is to determine how well eFI by itself will predict the 
outcomes
...
interventions to prevent health decline and poor outcomes. There is no gold
standard method to assess Frailty in clinical practice. Currently available
frailty assessment tools used in geriatric practice have good validity (for
example @fried01a) but these are time intensive and often difficult to implement
in a busy general practice. Because of the simplicity of the Rockwood Frailty
Index, it is more likely to be adopted by clinicians in a busy practice.

*[TRIPOD 15.b, development only] Explain how to the use the prediction model.*

The statistical analysis we used to validate eFI as a predictor of 
infections, trauma, and cardiac complications R [@rteam21] is not 
a prerequisite for _deploying_ eFI in the clinic. Cox models can be useful 
initially to find optimal thresholds tuned to specific outcomes instead of 
relying on the 0.19 cutoff but here we showed that this cutoff is effective at 
partitioning patients into risk groups for very different types of 
PSIs. Nor is eFI limited to any specific EHR system. As long as 
it is possible to extract a table of dates, patient IDs, ICD-10 codes, and LOINC 
codes for labs with accompanying value-flags, such a result-set can be converted 
into eFI values at patient-visit granularity and used as a predictor. We 
publicly shared mapping tables and scripts [@bokov20] to use as a starting point 
for researchers or enterprise analytics teams that wish to deploy eFI at their 
own health systems.
...
This contributes to a growing body of evidence that
...
code mappings but also scripts [@bokov20] that implement the mapping algorithm 
with notes on adapting them to local environments. We intend to evolve these
