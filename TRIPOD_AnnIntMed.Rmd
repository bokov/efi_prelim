...
*[TRIPOD 5.a] OK Specify key elements of the study setting (e.g., primary care, secondary care, general population) including number and location of centres.*  
...
tb1 <- bind_rows(dat04[,.SD,.SDcols=.dat04keep]
  ...
  mutate(across(everything(),~({
    label(.)<-paste0('<b>',rname(cur_column()),'</b>'
...
validation data, `r tb1df[11,7]` were. As expected, the majority of patients had 
...
```{r kmpsi, fig.cap=NULL, fig.height=4, fig.width=12, message=FALSE,collapse='true',results='asis'}
...
```
```{r kmpsicap, fig.cap='Any patient safety event.',fig.height=0.1}
.oldmar <- par('mar'); par(mar=c(0,0,0,0));
plot(0,type='n',axes=F,xlab=NA,ylab=NA,cex=0.01);
par(mar=.oldmar);
```

Lorem ipsum regular text ...

```{r kminf, fig.cap=NULL, fig.height=4, fig.width=12, message=FALSE,collapse='true',results='asis'}
panderOptions('knitr.auto.asis', FALSE);
.subfig <- 0;
...
```{r kmtra, fig.cap=NULL, fig.height=4, fig.width=12, message=FALSE,collapse='true',results='asis'}
panderOptions('knitr.auto.asis', FALSE);
.subfig <- 0;
...
We used R [@rteam21] and the Survival R package [@therneau00] for all analysis. 
We fit very simple univariate Cox models that in R syntax have the following 
specification:
...
eFI. All model coefficients are shown in Figures 
\@ref(fig:kmpsicap)-\@ref(fig:kmcardcap).
...
a preqrequisite for _deploying_ eFI in the clinic. Cox models can be useful 
initially to find optimal thresholds tuned to specific outcomes instead of 
relying on the 0.19 cutoff but here we showed that this cutoff does an 
good job of partitioning patients into risk groups for very different types of 
patient safety events. Nor is eFI limited to any specific EHR system. As long as 
...
publicly shared code [@bokov20] to use as a starting point for researchers or 
enterprise analytics teams to deploy their own local eFI.
...
,"Did I get everybody's correctly?",date='2020-12-04T14:20')` The funders of 
this study had no influence on the design, data collection, analysis, 
