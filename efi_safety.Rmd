...
.outcomenames <- c('vi_c_psi','vi_c_hospinfect','vi_c_hosptrauma'
                   ,'vi_c_cardiac');
snip_outcomes <- submulti(.outcomenames,dct0[,c('colname','dispname')]);
...
Johnston et al. {Johnston, 2020} reported that frailty can improve the accuracy
...
index to EMR-derived data{Clegg, 2016}{Pajewski, 2019}{Stow, 2018}. However, so
far only patients older than 65 have been studied. Yet value-based care is
broader than Medicare as `r fs('private payers start moving to the ACO
model',class='note2team')`, so it is important to understand whether EFI
generalizes to outcomes such as
preventable safety events in other age groups.
...
health and healthcare.” {Institute of Medicine (IOM). Digital infrastructure for
the learning health system: the foundation for continuous improvement in health
and health care. In: Grossman C, Powers B, McGinnis JM, eds. The foundation for
continuous improvement in health and health care workshop series summary.
National Academies Press, 2011: 1–311}.
...
difficult to interpret as survival curves but nevertheless are shown in `r
fs('supplementary table S1')`. The remaining 10 patient safety indicators were
too rare to reliably analyze in a sample this size taken from a _general_
...
```{r tb3}
tb3 %>% mutate(Outcome = submulti(Outcome,dct0[,c('colname','dispname')])) %>%
